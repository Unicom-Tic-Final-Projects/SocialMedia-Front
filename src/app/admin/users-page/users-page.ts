...
// Extended interface for mapped user data with display properties
interface UserDisplayData extends AdminUserResponse {
  id: string;
  name: string;
  status: string;
  joined: string;
...
export class AdminUsersPage implements OnInit, OnDestroy {
  ...
  editingUser: UserDisplayData | null = null;
  saving = signal(false);
  ...
  trackByUserId(index: number, user: UserDisplayData): string | number {
    ...
    this.usersService
      ...
      next: (response) => {
        ...
        if (response.success && response.data) {
          this.users = response.data.map((user): UserDisplayData => ({
            ...user,
            id: user.userId,
  ...
  loadTenants() {
    this.usersService
      ...
      error: (error: unknown) => {
        this.loggingService.error('Error loading tenants', error, 'AdminUsersPage');
      },
    });
  }
  ...
  openEditModal(user: UserDisplayData) {
  ...
  saveTenant() {
    ...
    const tenantName =
      formValue.displayName?.trim() || formValue.email?.split('@')[0] || formValue.email;
    ...
    this.usersService.createTenant(createRequest).subscribe({
      ...
      error: (error: unknown) => {
        this.loggingService.error('Error creating tenant', error, 'AdminUsersPage');
        this.savingTenant.set(false);
        // Extract error message safely
        const httpError = error as { error?: { message?: string }; message?: string };
        const errorMsg =
          httpError?.error?.message || httpError?.message || 'Failed to create tenant';
  ...
  saveUser() {
    ...
    if (this.isEditMode() && this.editingUser) {
      ...
      this.usersService.updateUser(this.editingUser.id, updateRequest).subscribe({
        ...
        error: (error: unknown) => {
          this.loggingService.error('Error updating user', error, 'AdminUsersPage');
          const httpError = error as { error?: { message?: string } };
          this.errorMessage.set(httpError.error?.message || 'Failed to update user');
          this.saving.set(false);
        },
      });
    } else {
      ...
      this.usersService.createUser(createRequest).subscribe({
        ...
        error: (error: unknown) => {
          this.loggingService.error('Error creating user', error, 'AdminUsersPage');
          const httpError = error as { error?: { message?: string } };
          this.errorMessage.set(httpError.error?.message || 'Failed to create user');
          this.saving.set(false);
        },
      });
    }
  ...
  deleteUser(id: string | number) {
    if (confirm('Are you sure you want to delete this user?')) {
      this.usersService.deleteUser(id.toString()).subscribe({
        ...
        error: (error: unknown) => {
          this.loggingService.error('Error deleting user', error, 'AdminUsersPage');
